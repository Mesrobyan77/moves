...
function Trailer({ movieId, type = "movie", seasonsData = [] }: PlayerProps) {
  ...
  const getVidsrcUrl = (version: string) => {
    ...
    if (type === "anime") return `${baseUrl}/anime/tmdb:${movieId}/${e}/sub${params}`;
            ...
            <div className="flex gap-3">
              ...
              {validSeasons.length > 1 && (
                <div className="group flex items-center gap-2 bg-background/60 px-4 py-2 rounded-xl border border-border hover:border-primary/50 transition-all shadow-inner">
                  <span className="text-[10px] text-primary font-black uppercase tracking-tighter">S</span>
                  <select
                    value={s}
                    onChange={(evt) => {
                  ...
                  >
                    {validSeasons.map((season) => (
                      <option key={season.id} value={season.season_number} className="bg-card text-foreground">
                        {season.season_number < 10 ? `0${season.season_number}` : season.season_number}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              {/* Episode Select */}
              <div className="group flex items-center gap-2 bg-background/60 px-4 py-2 rounded-xl border border-border hover:border-primary/50 transition-all shadow-inner">
                <span className="text-[10px] text-primary font-black uppercase tracking-tighter">E</span>
                <select
                  value={e}
