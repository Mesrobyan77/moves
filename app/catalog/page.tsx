        ...
        <div className="bg-card p-4 rounded-2xl shadow-2xl border border-border flex flex-col lg:flex-row gap-4 items-center">
          <div className="flex flex-wrap gap-3 w-full lg:flex-1">
            ...
            <div className="relative w-full lg:w-72 group order-first lg:order-none">
              <FiSearch className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${searchError ? "text-red-500" : "text-muted group-focus-within:text-primary"}`} />
              <input
                type="text"
                value={searchQuery}
              ...
              />
              {searchQuery && (
                <FiX onClick={() => { setSearchQuery(""); setSearchError(null); }} className="absolute right-4 top-1/2 -translate-y-1/2 text-muted cursor-pointer hover:text-foreground" />
              )}
            </div>
            ...
            <div className="relative flex-1 min-w-[130px] lg:flex-none group cursor-pointer bg-background border border-border px-4 py-2.5 rounded-xl flex items-center justify-between hover:border-primary/40 transition-all">
              <span className="text-[11px] text-muted font-bold uppercase tracking-wider truncate max-w-[100px]">{genreLabel}</span>
              ...
              <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-2xl max-h-60 overflow-y-auto">
                <ul className="text-xs">
                  <li onClick={() => setSelectedGenre(null)} className="px-4 py-3 hover:bg-primary hover:text-black border-b border-border transition-colors">{t('catalog.all_genres')}</li>
                  {genres.map((g) => (
                    <li key={g.id} onClick={() => setSelectedGenre(g.id.toString())} className="px-4 py-3 hover:bg-primary hover:text-black border-b border-border transition-colors">{g.name}</li>
                  ))}
                </ul>
              </div>
            </div>
            ...
            <div className="relative flex-1 min-w-[130px] lg:flex-none group cursor-pointer bg-background border border-border px-4 py-2.5 rounded-xl flex items-center justify-between hover:border-primary/40 transition-all">
              <span className="text-[11px] text-muted font-bold uppercase tracking-wider">
                {selectedRating ? `${selectedRating}+ ${t('catalog.stars')}` : t('catalog.rating')}
              </span>
              <FiChevronDown className="text-muted group-hover:text-primary" />
              <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-2xl">
                <ul className="text-xs">
                  <li onClick={() => setSelectedRating(null)} className="px-4 py-3 hover:bg-primary hover:text-black border-b border-border transition-colors">{t('catalog.any_rating')}</li>
                  {[9, 8, 7, 6, 5].map((num) => (
                    <li key={num} onClick={() => setSelectedRating(num)} className="px-4 py-3 hover:bg-primary hover:text-black border-b border-border transition-colors">{num}+ {t('catalog.stars')}</li>
                  ))}
                </ul>
              </div>
            </div>
            ...
            {/* SORT DROPDOWN */}
            <div className="relative flex-1 min-w-[130px] lg:flex-none group cursor-pointer bg-background border border-border px-4 py-2.5 rounded-xl flex items-center justify-between hover:border-primary/40 transition-all">
              <span className="text-[11px] text-muted font-bold uppercase tracking-wider truncate max-w-[100px]">{sortLabel}</span>
              <FiChevronDown className="text-muted group-hover:text-primary" />
              <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-2xl overflow-hidden">
                <ul className="text-xs">
                  {sortOptions.map((opt) => (
                    <li key={opt.value} onClick={() => setSelectedSort(opt.value)} className="px-4 py-3 hover:bg-primary hover:text-black border-b border-border transition-colors">{opt.label}</li>
                  ))}
                </ul>
              </div>
            </div>
      ...
      <section className="container mx-auto px-4 min-h-[600px]">
        {searchError ? (
          <div className="text-center py-40 bg-red-500/5 rounded-3xl border border-dashed border-red-500/20">
            ...
            <p className="text-red-500 font-bebas text-2xl tracking-widest uppercase">{t('catalog.restricted')}</p>
            <p className="text-red-500/60 text-sm mt-2 font-bold">{searchError}</p>
          </div>
        ) : loading ? (
          ...
          <>
            ...
            {displayMovies.length === 0 && (
              <div className="text-center py-40 text-muted font-bebas text-xl tracking-widest uppercase">{t('catalog.no_results')}</div>
            )}
            {!isSearching && (
...
// Հեռացվել է label-ը set անելու ավելորդ logic-ը
function Dropdown({ label, options, onSelect }: { label: string, options: {label: string, value: string}[], onSelect: (val: string) => void }) {
    return (
        <div className="relative flex-1 min-w-[130px] lg:flex-none group cursor-pointer bg-background border border-border px-4 py-2.5 rounded-xl flex items-center justify-between hover:border-primary/40 transition-all">
            ...
            <div className="absolute top-[110%] left-0 w-full bg-card border border-border rounded-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-2xl overflow-hidden">
                <ul className="text-xs">
                    {options.map((opt) => (
                        <li key={opt.value} onClick={() => onSelect(opt.value)} className="px-4 py-3 hover:bg-primary hover:text-black transition-colors border-b border-border">{opt.label}</li>
                    ))}
                </ul>
            </div>
        </div>
