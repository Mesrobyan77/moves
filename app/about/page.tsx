    ...
    <main className="min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <section className="bg-card py-24 border-y border-border">
        <div className="container mx-auto px-4">
          <h2 className="text-4xl font-bebas text-center mb-16 uppercase tracking-wider">
      ...
      <section className="container mx-auto px-4 py-24">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
          <Feature
            icon={<FiStar />}
            title={t("about.features.ultra_hd")}
            desc={t("about.features.ultra_hd_desc")}
          />
          <Feature
            icon={<FiTv />}
            title={t("about.features.online_tv")}
            desc={t("about.features.online_tv_desc")}
          />
          <Feature
            icon={<FiActivity />}
            title={t("about.features.fast_loading")}
            desc={t("about.features.fast_loading_desc")}
          />
        </div>
      </section>
    </main>
...
function StatItem({ count, label }: { count: string; label: string }) {
  return (
    <div className="flex flex-col">
      ...
      <span className="text-[10px] uppercase tracking-widest text-muted font-bold mt-1">
        {label}
      </span>
    </div>
  );
...
function WorkStep({
  ...
  return (
    <div className="relative p-10 bg-background rounded-3xl border border-border hover:border-primary/30 transition-all group shadow-sm">
      <span className="absolute top-6 right-8 text-6xl font-bebas text-foreground/5 group-hover:text-primary/10 transition-colors">
        {step}
      </span>
      <div className="text-primary text-3xl mb-6">{icon}</div>
      <h4 className="text-xl font-bebas mb-4 tracking-wide uppercase">
        {title}
      </h4>
...
function Feature({
  ...
  return (
    <div className="flex gap-6 group">
      <div className="mt-1 text-primary text-2xl group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <div>
        <h4 className="text-lg font-bebas mb-3 tracking-wide uppercase">
          {title}
        </h4>
        <p className="text-muted text-sm leading-relaxed">{desc}</p>
      </div>
