...
function ContactInfoCard({
  ...
  return (
    <div className="bg-card p-8 rounded-3xl border border-border flex items-start gap-6 hover:border-primary/30 transition-all group shadow-sm">
      ...
      <div>
        <h4 className="text-muted text-[10px] uppercase tracking-widest font-black mb-2">
          {title}
        </h4>
        <p className="text-lg font-bebas tracking-wide mb-1 group-hover:text-primary transition-colors uppercase">
          {detail}
        </p>
...
function SocialLink({ icon }: { icon: React.ReactNode }) {
  return (
    <a
      href="#"
      className="w-10 h-10 rounded-lg bg-background border border-border flex items-center justify-center text-muted hover:border-primary hover:text-primary transition-all"
    >
      {icon}
    </a>
  );
